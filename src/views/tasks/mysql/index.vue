...
const getUserLists = ()=>{
    createIAxios("/api/mysql/userLists","POST", {id: instanceInfo.id}).then((res) =>{
        userLists.options = res.lists
    })
}
const OpenWanService = ()=>{
    createIAxios("/api/mysql/open","POST", {id: instanceInfo.id}).then((res) =>{
    })
}
const CloseWanService = ()=>{
    createIAxios("/api/mysql/close","POST", {id: instanceInfo.id}).then((res) =>{
    })
}
const updatePostgresPassword = ()=>{
    createIAxios("/api/mysql/user","POST", {username: choseUser.name,id:instanceInfo.id}).then((res) =>{
        dialogFormVisible.value = false
    })
}
const createUserReq = ()=>{
    createIAxios("/api/mysql/user","POST", {id: instanceInfo.id}).then((res) =>{
    })
}
